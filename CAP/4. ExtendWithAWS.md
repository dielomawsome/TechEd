...
> Note that this is now referencing an additional npm package, so in case this is not installed yet in your local setup it may raise an error can ba be fixed with `npm i xml2json` or by adding the information on file `package.json`
